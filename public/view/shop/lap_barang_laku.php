...
$pdf->SetX(4);            
$pdf->MultiCell(19.5,0.5,'Sport Market Playstation',0,'L');
$pdf->SetX(4);
...
$query=mysqli_query($con,"select * from buy where tanggal=" . $tanggal);
while($lihat=mysqli_fetch_array($query)){
	$pdf->Cell(1, 0.8, $no , 1, 0, 'C');
	$pdf->Cell(3, 0.8, $lihat['tanggal'],1, 0, 'C');
	$pdf->Cell(6, 0.8, $lihat['namabarang'],1, 0, 'C');
	$pdf->Cell(4, 0.8, "Rp. ".number_format($lihat['hargabarang'])." ,-", 1, 0,'C');
...
$q=mysqli_query($con,"select sum(total_harga) as total from buy where tanggal=".$tanggal);
...
$qu=mysqli_query($con,"select sum(laba) as total_laba from buy where tanggal=".$tanggal);
// select sum(total_harga) as total from barang_laku where tanggal='$tanggal'
