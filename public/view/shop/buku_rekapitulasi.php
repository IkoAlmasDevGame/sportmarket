...
<a href="rekapitulasi_barang.php"><span class="glyphicon glyphicon-arrow-left"></span>  Kembali</a>
<div class="mb-4"></div>
...
<form method="get">
	<div class="input-group col-md-5 col-md-offset-0">
		...
		<select type="submit" name="tanggal" class="form-control" onchange="this.form.submit()">
			...
			<?php
			$pil=mysqli_query($con,"SELECT * FROM buy");
...
<table class="table table-bordered">
    ...
    if(isset($_GET['tanggal'])){
        ...
        $bsale = mysqli_query($con, "SELECT * FROM buy where tanggal like '$tanggal' order by tanggal desc");
    }else{
        $bsale = mysqli_query($con, "select * from buy order by tanggal desc");
    }
    $no = 1;
    while($s = mysqli_fetch_array($bsale)){
        ...
        <tr>
            ...
            <td><?php echo "Rp.".number_format($s['hargabarang']);?></td>
    ...
    <tr>
		...
		if(isset($_GET['tanggal'])){
			$tanggal = $_GET['tanggal'];
			$x=mysqli_query($con,"SELECT sum(total_harga) as total from buy where tanggal='$tanggal'");				
	...
	<tr>
		...
		if(isset($_GET['tanggal'])){
			$tanggal = $_GET['tanggal'];
			$x=mysqli_query($con,"SELECT sum(laba) as total from buy where tanggal='$tanggal'");			
			if($xx=mysqli_fetch_array($x)){
				echo "<td><b> Rp.". number_format($xx['total']).",-</b></td>";
			}else{}
		}
